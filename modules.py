...
class ComplexEdgeDetector(nn.Module):
    def __init__(self, channels, kernel_size=3, padding=1):
        super(ComplexEdgeDetector, self).__init__()
        # 高斯平滑
        self.gaussian = nn.Conv2d(channels, channels, kernel_size=5, padding=2, bias=False)
        ...
        # 修改高斯核的形状以匹配输入通道数
        gaussian_kernel = gaussian_kernel.unsqueeze(0).unsqueeze(0).repeat(channels, channels, 1, 1)
        self.gaussian.weight.data = gaussian_kernel

        # 拉普拉斯算子
        self.laplacian = nn.Conv2d(channels, channels, kernel_size=3, padding=1, bias=False)
        laplacian_kernel = torch.tensor([[0, 1, 0],
                                         [1, -4, 1],
                                         [0, 1, 0]], dtype=torch.float32)
        laplacian_kernel = laplacian_kernel.unsqueeze(0).unsqueeze(0).repeat(channels, channels, 1, 1)
...
class DMRM(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.ir_embed = nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1),
                                      nn.ReLU())
        self.vi_embed = nn.Sequential(nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1),
                                      nn.ReLU())
        ...
        self.grad_ir = ComplexEdgeDetector(out_channels)
        self.grad_vi = ComplexEdgeDetector(out_channels)
...
class Fuse_block(nn.Module):
    def __init__(self, dim, channels=32):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(dim, channels, kernel_size=3, padding=1),
            nn.ReLU(),
            ResidualBlock(channels)  # 新增残差块
        )
        self.down_conv = nn.Sequential(
            nn.Conv2d(channels, channels*2, kernel_size=3, stride=2, padding=1),  # 下采样
            nn.ReLU(),
            nn.Conv2d(channels*2, channels, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Conv2d(channels, 1, kernel_size=1),  # 简化输出层
            nn.Tanh()
        )
    ...
    def forward(self, ir, vi, frefus):
        ...
        x = self.encoder(x)
        x = F.interpolate(x, scale_factor=2, mode="bilinear")  # 上采样恢复分辨率
        x = self.down_conv(x)
        return x


class IFFT(nn.Module):
    def __init__(self, out_channels=16):  # 增加通道数
        super().__init__()
        self.conv1 = nn.Sequential(
            nn.Conv2d(2, out_channels, kernel_size=3, padding=1),
            nn.ReLU(),
            Att_Block(out_channels, out_channels),  # 加入注意力
